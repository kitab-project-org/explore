...
function cleanDiffHtml(s) {
  ...
  s = s.replace(
    /(?<=<span class="(\w+)">)([^<]+)<\/span><span class="\1">/g,
    "$2"
  );
...
function markupHeckelArray(toks, toksArr, otherArr, xHtml, n, spanClass) {
  ...
  for (let i = 0; i < toks.length; i++) {
    ...
    if (typeof toksArr[i] === "string") {
      // no equivalent found in other string N
      ...
      if (usedChars == 0) {
        ...
        // characters have already been added to the previous tag!
        usedChars--;
      }
    }
  }
...
function refine(O, N, aHtml, bHtml, nextChars, intoRows, arChars, refine_n) {
  ...
  var aLastChunk = aHtml.split("###NEW_ROW###")[aHtml.split("###NEW_ROW###").length-1]
  var bLastChunk = bHtml.split("###NEW_ROW###")[bHtml.split("###NEW_ROW###").length-1]

  // Add a splitter to make comparing texts easier:
  if (
    intoRows &&
    (countArChars(aLastChunk) > arChars || countArChars(bLastChunk) > arChars) &&
    //(countArChars(aHtml) > arChars || countArChars(bHtml) > arChars) &&
...
function parseDiffHtml(diffHtml, intoRows, arChars, refine_n) {
  ...
  for (var i = 0; i < rootNode.childNodes.length; i++) {
    ...
    if (c.nodeType === Node.TEXT_NODE) {
      ...
      c.classList.contains("wikEdDiffMarkLeft")
    ) {
